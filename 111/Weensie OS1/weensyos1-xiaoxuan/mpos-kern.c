...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		/*pid_t w = current->p_waiting;
		pid_t j = 1;
		while(j<NPROCS){
		if(current ->p_waiting == proc_array[j].p_waiting)
		{
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
		...
		else if (proc_array[p].p_state == P_ZOMBIE)
			{
				...
				proc_array[p].p_state = P_EMPTY;//in blocking mode, p_zombie
			}
		else
		{
...
copy_stack(process_t *dest, process_t *src)
{
	...
	src_stack_top = PROC1_STACK_ADDR + ((int)(src->p_pid))*PROC_STACK_SIZE; 
	size_t src_stack_size = src_stack_top - src_stack_bottom;
	//memcpy()
	dest_stack_top = PROC1_STACK_ADDR + ((int)(dest->p_pid))*PROC_STACK_SIZE; /* YOUR CODE HERE */;
	dest_stack_bottom = dest_stack_top - src_stack_size;/* YOUR CODE HERE: calculate based on the
				 other variables */;
	memcpy((void *)dest_stack_bottom,(const void *)src_stack_bottom,src_stack_size);
