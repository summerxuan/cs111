...
void 
delete_check_deadlock_list (pid_t current_pid, osprd_info_t *d)
{
	if (current_pid <0 | d==NULL)
	{
	...
	else
	{
		...
		if (curr ==d-> check_deadlock_list_head && curr== d->check_deadlock_list_tail)
		{
			d->check_deadlock_list_head = NULL;
			d->check_deadlock_list_tail = NULL;
			kfree(curr);
		}
		else if (curr == d->check_deadlock_list_head)
		{
			d->check_deadlock_list_head = d->check_deadlock_list_head->next;
			d->check_deadlock_list_head->prev = NULL;
			kfree(curr);
		}
		else if (curr == d->check_deadlock_list_tail)
		{
			d->check_deadlock_list_tail =d-> check_deadlock_list_tail ->prev;
