...
struct pid_node
	{
		pid_t pid;
		//pid_list_t children;
		pid_node_t children_head;
		pid_node_t children_tail;
...
static int osprd_close_last(struct inode *inode, struct file *filp)
{
	if (filp) {
		...
		if (filp->f_flags & F_OSPRD_LOCKED)
		{
			...
			{
				d->number_read_lock--;
				if (clear_current_reader(pid_list_head, pid_list_tail) == -ENOTTY)
					return -ENOTTY;
			}
		}
...
initChild(pid_t pid)
{
  pid_node_t new_pid =(pid_node_t)kmalloc(sizeof(struct pid_node),GFP_ATOMIC);
  //void *kmalloc(size_t size, gfp_t flags)	
  //Allocate size bytes of memory. Returns a pointer to the allocated memory. 
  //flags specifies details about the allocation; for the purposes of this lab, you should generally give GFP_ATOMIC.
  new_pid->pid = pid;
...
pid_node_t
initPidnode(pid_t curr)
{
	pid_node_t new_pid_node = (pid_node_t)kmalloc(sizeof(struct pid_node),GFP_ATOMIC);
	new_pid_node->pid = curr;
	new_pid_node->children_tail = NULL;
	new_pid_node->children_head = NULL;
	new_pid_node->status = WHITE;
	return new_pid_node;
}

void
add_children_pid(pid_node_t pid_n, pid_t children)
{
	//pid_node pid_n = initPidnode(curr)
	if(pid_n->children_tail==NULL)
	{
		pid_n->children_tail = initPidnode(children);
		pid_n->children_head = pid_n->children_tail; 
	}
	else
  {
    pid_node_t pid_new = initPidnode(pid);
    pid_n->children_tail->next = pid_new;
    pid_n->children_tail = pid_n->children_tail->next;
    pid_n->children_tail->next = NULL;
  }
 // return pid_n;
}
int
...
{
	if(p->status = GREY)
		return 0;//indicate the deadlock exits;
	//visit(p);

  p->status = GREY; //to indicate this node is being visited
  pid_node_t pid_child_now = p->children_head;
  while( pid_child_now != NULL)
    {
    if (pid_child_now->status == WHITE) //if the child is unexplored
      visit(pid_child_now);
  	pid_child_now = pid_child_now->next;
	}
	pid_child_now->status = BLACK; //to show we're done visiting this node
  return 1;
}
int
check_deadlock(struct file *filp)
{
	osprd_info_t *d = file2osprd(filp);
	int i; 
	i = 0;	
	while(i<d->array)
	{
		pid_node_t p = d->pid_node_array[i];
		if(visit(p)==0)
			return -EDEADLK;
		i++;
	}
...
int 
osprd_ioctl(struct inode *inode, struct file *filp,
		unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == OSPRDIOCACQUIRE) {
		...
		if(filp_writable)
		{
			...
			osp_spin_lock(&d->mutex);
			initilize(filp);
			if(check_deadlock(filp) == -EDEADLK)
				return -EDEADLK;
			pid_node_t pid_node_now;
			pid_node_now= initPidnode(current->pid);
			add_children_pid(pid_node_now, d->write_lock_holder);
			pid_list_t pid_now = pid_list_head;
			while(pid_now != NULL)
			{
				add_children_pid(pid_node_now, pid_now->pid);
				pid_now = pid_now->next;
			}
			d->pid_node_array[d->array] = pid_node_now;
			d->array++;
			osp_spin_unlock(&d->mutex);

			int w; 
			w = wait_event_interLruptible(d->blockq, (d->number_write_lock==0&&d->number_read_lock==0&&cur_ticket==d->ticket_tail));
		...
		{
			osp_spin_lock(&d->mutex);
			pid_node_t pid_node_now = initPidnode(current->pid);
			add_children_pid(pid_node_now,d->write_lock_holder);
			//pid_list_t pid_now = pid_list_head;
			d->pid_node_array[d->array] = pid_node_now;
			d->array++;
			...
			int w ;
			w = wait_event_interruptible(d->blockq, (d->number_write_lock==0&&cur_ticket==d->ticket_tail));
			//Blocks the current task on a wait queue until a CONDITION becomes true. 
			//A request for a write lock on a ramdisk file will block until no other files on that 
			//ramdisk have a read or writeloc
			if(w == -ERESTARTSYS)
