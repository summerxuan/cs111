...
delete_check_deadlock_list (pid_t current_pid, osprd_info_t *d)
{
	...
	if (d->check_deadlock_list_head ->pid != current_pid)
	{
	...
	{
		if (d->check_deadlock_list_head == d->check_deadlock_list_tail)
			kfree(d->check_deadlock_list_head);
		else 
		{
			pid_list_t tmp = d->check_deadlock_list_head;
