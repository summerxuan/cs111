...
// last copy is closed.)
int 
check_list_not_empty(osprd_info_t *d)
{
	if (d->check_deadlock_list_head != NULL)
		return 1;
	else 
		return 0;
}
void
clean_list(osprd_info_t *d)
{
	pid_list_t curr = d->check_deadlock_list_head;
	pid_list_t prev = d->check_deadlock_list_head;
	while (curr != NULL)
	{
		curr = curr->next;
		kfree(prev);
 ...
 */
pid_list_t
initPidlist(pid_t pid,int type)
{
...
add_read_pid(pid_t pid, osprd_info_t *d)
{
  ...
  {
    d->pid_list_tail = initPidlist(pid,0);
    d->pid_list_head= d->pid_list_tail;
  }
  else
  {
    pid_list_t pid_new = initPidlist(pid,0);
...
*/
void
add_check_deadlock_list(pid_t pid, osprd_info_t *d ,int type)
{
  
  if(d->check_deadlock_list_tail== NULL)
  {
    d->check_deadlock_list_tail = initPidlist(pid,type);
    d->check_deadlock_list_head= d->check_deadlock_list_tail;
  }
  else
  {
    pid_list_t pid_new = initPidlist(pid,type);
    //command_stream_head->next = command_stream_new;
...
int pid_in_list(pid_list_t p_head , pid_t pid)
{
	pid_list_t curr = p_head;
	//pid_list_t curr_tail = p_tail;
		//left to right order
		while(curr!=NULL)
		{
			if(curr->pid == pid)//find the existing one
			{
				return 1;
				break;
			}
...
int
pid_in_list_write(pid_list_t p_head , pid_t pid)
{
	pid_list_t curr = p_head;
	//pid_list_t curr_tail = p_tail;
		//left to right order
		while(curr!=NULL)
		{
			if(curr->pid == pid && curr->permit == W)//find the existing one
			{
				return 1;
				break;
			}
...
int find_pid_list(pid_list_t head,pid_t pid)
{
	pid_list_t curr = head;
	int count = 1;
	while(curr != NULL)
...
int
find_until_count (pid_list_t od_head, pid_t current_pid, int count)
{
	if (od_head == NULL)
		return 0;	//没有找到
	int i;
	...
	while (tmp != NULL&&i<count)
	{
		...
		if (tmp->pid == current_pid)	//i <count
		{
				return 1;
		}
		tmp = tmp->next;
...
int check_deadlock(osprd_info_t *d)
{
	...
	if(pid_in_list(depend_head,current->pid))
	{	
		//eprintk ("pid in list");
		//osp_spin_unlock(&d->mutex);
		r = 1;
	...
	while(i < NOSPRD)
	{
		...
		if(p == NULL)
		{
			if(pid_in_list(od_head,current->pid))
				{
					//eprintk ("multi device");
		...
		while(p!=NULL)
		{
			...
			if(pid_in_list(od_head,p->pid))
				{
					int count = find_pid_list(od_head,p->pid);
					if(find_until_count(od_head,current->pid,count))
						{

							//eprintk ("find in other");
							//osp_spin_unlock(&od->mutex);
							r =  1;
							break;
						}
...
int check_deadlock_read(osprd_info_t *d)
{
	//osp_spin_lock(&d->mutex);
	pid_list_t depend_head = d->check_deadlock_list_head;
	pid_list_t depend_tail = d->check_deadlock_list_tail;
	...
	while(i < NOSPRD)
	{
		...
		{
			//eprintk("current: %d\n", (int)current->pid);
			if(pid_in_list_write(od_head,current->pid))
				{
					 r = 1;
				}
		}
		...
		{
			//eprintk ("in second while loop");
			if(pid_in_list_write(od_head,p->pid))
				{
					int count = find_pid_list(od_head,p->pid);
					if(find_until_count(od_head,current->pid,count))
						{
		...
		unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == OSPRDIOCACQUIRE) {
		if (d->write_lock_holder == current->pid)
			return -EDEADLK;
		//int k =0 ;
		pid_list_t curr = d->pid_list_head;
		while (curr != NULL) {
			//pid_list_t curr = d->pid_list_head;
		...
		{
			add_check_deadlock_list(current->pid, d,1);
	...
	else
	{
		...
		{
			add_check_deadlock_list(current->pid, d,0);
			//eprintk ("add_check_deadlock_list");
			cur_ticket = d->ticket_head;
			d->ticket_head ++ ;
		}
...
static void osprd_setup(osprd_info_t *d)
{
	...
	d->check_deadlock_list_tail = NULL;
	d->check_deadlock_list_head = NULL;
	//d->write_lock_holder = -1;
	d->pid_list_head= NULL;
	d->pid_list_tail =NULL;
