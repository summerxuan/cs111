...
delete_check_deadlock_list (pid_t current_pid, osprd_info_t *d)
{
	...
	else
	{
		...
		if (curr == check_deadlock_list_head && curr== check_deadlock_list_tail)
		{
			check_deadlock_list_head = NULL;
			check_deadlock_list_tail = NULL;
			kfree(curr);
		}
		else if (curr == check_deadlock_list_head)
		{
			check_deadlock_list_head = check_deadlock_list_head->next;
			check_deadlock_list_head->prev = NULL;
			kfree(curr);
		}
		else if (curr == check_deadlock_list_tail)
		{
			check_deadlock_list_tail = check_deadlock_list_tail ->prev;
			check_deadlock_list_tail->next = NULL;
			kfree (curr);
		}
		else
		{
...
int pid_in_list(pid_list_t p_head , pid_list_t p_tail, pid_t pid, int type)
{
	...
	else
	{
		...
		{
			if(curr_tail->pid == pid)//find the existing one
			{
				return 1;
				break;
			}
...
int find_pid_list(pid_list_t head, pid_list_t tail, pid_t pid)
{
		...
		unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == OSPRDIOCACQUIRE) {
		...
		else
		{
			...
			osp_spin_unlock (&d->mutex);
			wake_up_all (&d->blockq);
			eprintk ("before delete_check_deadlock_list");
