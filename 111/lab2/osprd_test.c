...
void
clear_readers (pid_t pid_list_head, pid_t pid_list_tail)
{
	pid_list_t pid_now = pid_list_head;
	while (pid_now!= pid_list_tail)
	{
		//pid_t tmp = pid_list_head;
		pid_list_now ->pid = -1;
		pid_list_now = pid_list_now->next;
		//free (tmp);
	}
	//free (pid_list_tail);
}
	...
	/*	if (filp->f_flags & F_OSPRD_LOCKED)
		{
			...
			{
				d->number_write_lock --;
				if (d->number_read_lock == 0)
					wake_up_all (&(d->blockq));
				// else  
					//wake_up_read_requesters();
		...
		unsigned int cmd, unsigned long arg)
{
	...
	unsigned cur_ticket = d->ticket_head;
	d->ticket_head ++ ;
	...
	/*	osp_spin_lock (&d->mutex);
		...
		else
		{
			...
			else 	//release the read locker
			{
				...
				clear_readers (pid_list_head, pid_list_tail);
			}
			filp->f_flags &= ~F_OSPRD_LOCKED; 
			osp_spin_unlock (&d->mutex);
			wake_up_all (&d->blockq);
