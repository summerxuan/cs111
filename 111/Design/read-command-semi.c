   ...
   // printf("the %d time loop, the type is %d", counter,token_now->t_type);
    ...
    
    {
      ...
      while (  !is_empty(operators) && top(operators)->type == PIPE_COMMAND )
        ...
        case WORD:
        if (strcmp (token_now->text, "until") == 0 ||strcmp (token_now->text, "while") == 0 ||strcmp (token_now->text, "for") == 0)
          {
            //comm->type = UNTIL_COMMAND;
            char* keyword = token_now->text;
            token_stream_t condition = (token_stream_t)checked_malloc(sizeof( token_stream_t ));
            ...
            //loopbody
            token_stream_t loopBody = (token_stream_t)checked_malloc(sizeof( token_stream_t ));
          ...
          else if(token_now->t_type == WORD &&strcmp(token_now->text, "if")==0)
        {
          ...
          token_now = token_now->next;
          token_stream_t new =(token_stream_t)checked_malloc(sizeof(token_stream_t));
          ...
          //then....else
          token_stream_t ifBody =  (token_stream_t)checked_malloc(sizeof( token_stream_t ));
          ...
          if (token_now->t_type == WORD &&(strcmp (token_now->text, "else") ==0 || strcmp (token_now->text, "fi") ==0))
            ...
            {
              token_stream_t elseBody =  (token_stream_t)checked_malloc(sizeof( token_stream_t ));
