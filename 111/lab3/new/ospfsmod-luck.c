	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		/* EXERCISE: Your code here */
			...
			if (od->od_ino!=0)
		 {
		 	...
		 	else if (entry_oi->oi_ftype == OSPFS_FTYPE_DIR)		//dir
		 		ok_so_far = filldir(dirent, od->od_name, strlen(od->od_name), f_pos, od->od_ino, DT_DIR);
		 	else if(entry_oi->oi_ftype == OSPFS_FTYPE_SYMLINK)	//simlink
...
direct_index(uint32_t b)
{
	...
	if(b< OSPFS_NDIRECT)
		return b;
	//return the index in direct table directly
	else
	{
		b -= OSPFS_NDIRECT;
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	if (src_dentry->d_name.len > OSPFS_MAXNAMELEN)
		return -ENAMETOOLONG;
	if (find_direntry(dir_oi, src_dentry->d_name.name, src_dentry->d_name.len))
		return -EEXIST;
	...
	memcpy (new_dentry->od_name, src_dentry->d_name.name, src_dentry->d_name.len);
	new_dentry->od_name[src_dentry->d_name.len]='\0';
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	ospfs_inode_t * link = (ospfs_symlink_inode_t*) ospfs_inode(entry_ino);
