...
command_stream_t command_stream_head;
command_stream_t command_stream_tail;
...
token_stream_t convert_buffer_to_token_stream(char* buffer, int size)
{
...
{
	return s->c[s->n-1];//return the top command_t 
}
...
{
	if(s->top == STACK_SIZE -1)		//full stack
	{
		return -1;		//indicating false
	}
	s->top += 1;
...
command_t make_command_tree(token_stream_t t)
{
	...
	{
		if( !(token_now->type == LEFT || token_now->type == RIGHT) )
    	{
      	// make new command
		...
		case AND:
			...
			while (  !is_empty(ops) &&( top(ops)->type == PIPE_COMMAND || top(ops)->type == OR_COMMAND || top(ops)->type == AND_COMMAND ))
        	{
        		command_t pop_ops = pop(ops);//pop until the top ops has smaller presendence than
        		if (!make_branch(pop_ops, operands))
          		{
        ...
        case PIPELINE:
			...
			while (  !is_empty(ops) && top(ops)->type == PIPE_COMMAND )
        	{
        		command_t pop_ops = pop(ops);//pop until the top ops has smaller presendence than
        		if (!make_branch(pop_ops, operands))
          		{
        ...
        case WORD:
        	...
        	while(count!=NULL&&count->next!=NULL&&count->next->type==WORD)//while next is still word
        	{
        		count = count ->next;
        		num_words++;
        	}
        ...
        make_command_tree(
          convert_buffer_to_token_stream(token_now->content, strlen(token_now->content))
          );
      ...
      case LEFT:
        ...
        token_now= token_now->next;
        if (token_now->type == WORD) // followed by a word
        {
...
{
	command_stream_t cmd_stream = (cmd_stream_t)checked_malloc(sizeof(cmd_stream_t))；
	cmd_stream->next = NULL;
	cmd_stream->prev = NULL;
	return cmd_stream;
}
void add_command_to_stream(command_stream_t command_stream_s, command_t new_command_t )
{
	//if the link list is empty
	if(command_stream_tail == NULL)
	{
		command_stream_s->p_command = new_command_t;
		command_stream_head = command_stream_first;
		command_stream_tail = command_stream_first;
		command_stream_s->next = NULL;
		command_stream_s->prev = NULL
	}
	else
	{
		command_stream_new = init_command_stream();
		command_stream_s->next = command_stream_new;
		command_stream_new->prev = command_stream_s;
		command_stream_new->next = NULL;
...
command_stream_t make_command_stream (int (*get_next_byte) (void *), void *get_next_byte_arg)
{
	...
	do
	{
		char ch = get_next_byte(get_next_byte_arg);
	...
	add_command_to_stream(command_stream_tail, new_command_t );
