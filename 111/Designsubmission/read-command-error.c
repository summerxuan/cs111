        ...
        if(curr_token->t_type ==LEFT_DERCTION||curr_token->t_type ==RIGHT_DERCTION)
          ...
          curr_stream->next = (token_stream_t)checked_malloc(sizeof(token_stream_t));
                curr_stream = curr_stream->next;
                 token_t now = create_token_with_type(HEAD, NULL, -1);
                 curr_stream->head = now;
                 curr_token ->next = now;
                 prev_token = curr_token;//assign the previous token to now
                 curr_token = curr_token->next;
                  curr_token->prev = prev_token;
...
token_stream_t
delete_newline(token_stream_t t)
{
  int delete = 0;
  token_t token_now = t->head;
  while(token_now!=NULL)
  {
    if(token_now->text == NULL)
      {
        token_now= token_now->next;
        continue;
      }
    else if(strcmp(token_now->text,"if")==0||strcmp(token_now->text,"for")==0||strcmp(token_now->text,"while")==0||strcmp(token_now->text,"until")==0)
    {
      delete =1;
      break;
    }
  ...
  if(delete == 1)
  {
    int nest  = 0;
   // int flag = 0;
    token_stream_t track = (token_stream_t)checked_malloc(sizeof( token_stream_t ));
  token_stream_t track_head = track;
  track->head = t->head;
  ...
  while(token_now != NULL)
  {
    if(token_now->t_type == WORD)
    {
      if(strcmp(token_now->text,"if")==0)//if the tokennow is if
      {
        nest = 1;
        token_now = token_now->next;
        if(token_now->text == NULL&&token_now->t_type == HEAD)
          {
            token_now->prev->next = token_now->next;
            token_now->next->prev = token_now->prev;
      ...
      if(strcmp(token_now->text,"if")==0)//if the tokennow is if
          ...
          token_now = token_now->next;
           if(token_now == NULL)//whicn means we reached the end
            {
              flag = 1;
              break;
            }
          if(token_now->text == NULL&&token_now->t_type == HEAD)
          {
            if(token_now->next == NULL)//whicn means we reached the end
            {
              flag = 1;
              break;
            }
          ...
          if(token_now->text == NULL)
          {
           ...
           if(strcmp(token_now->text,"fi")==0)
        {
          nest -- ;
          if(nest == 0)
          {
          track->tail = token_now;
            track->next = t->next;
            track= track ->next;
            t=t->next;
          }
        }
         if(token_now->next == NULL && nest> 0)//if read to the last token the nest still above zero
        {
           error(2, 0, " if and fi are not in pairs.");
                return NULL;
        }
        }
      ...
      else if((strcmp(token_now->text,"while")==0)||(strcmp(token_now->text,"for")==0)||(strcmp(token_now->text,"until")==0))//if the tokennow is if
      {
        nest = 1;
        token_now = token_now->next;
        if(token_now->text == NULL&&token_now->t_type == HEAD)
        ...
        while(nest>0)
        {
         if((strcmp(token_now->text,"while")==0)||(strcmp(token_now->text,"for")==0)||(strcmp(token_now->text,"until")==0))//if the tokennow is if
      {
        nest ++;
      }
          token_now = token_now->next;
          ...
          if(strcmp(token_now->text,"done")==0)
        {
          nest --;
          if(nest == 0){
          track->tail = token_now;
        ...
        if(token_now->next == NULL && nest> 0)//if read to the last token the nest still above zero
        {
           error(2, 0, " while and done are not in pairs.");
                return NULL;
        }
        }
...
make_command_tree(token_stream_t t)
{
  ...
  //if(token_now->next == NULL)
   // printf("Heere\n");
  while((token_now!=NULL)&&(token_now->t_type != HEAD))
  {
    counter++;
   // printf("the %d time loop, the type is %d", counter,token_now->t_type);
    ...
    
    {
      ...
      while (  !is_empty(operators) && top(operators)->type == PIPE_COMMAND )
       ...
       case WORD:
       if(strcmp(token_now->text,"!")== 0)
       {
            token_stream_t following = (token_stream_t)checked_malloc(sizeof( token_stream));
            token_t following_head = create_token_with_type(HEAD, NULL, -1);
            token_now = token_now->next;
            following_head -> next = token_now;
            following->head = following_head;
            following->tail = following_head;
        if(token_now->t_type == SUBSHELL || token_now->t_type == WORD)
        {
          following->tail = token_now;
        }
        else
        {
                error(2, 0, "Line %d: Syntax error. ! can only be followed by word or subshell", line);
                return NULL;
        }
        command_t follow_cmd = make_command_tree(following);
        comm->u.command[0] = follow_cmd;
        comm->type = NOT_COMMAND;
        push (operands, comm);
        token_now = token_now->next;
       }
       else if (strcmp (token_now->text, "until") == 0 ||strcmp (token_now->text, "while") == 0 ||strcmp (token_now->text, "for") == 0)
          {
            //comm->type = UNTIL_COMMAND;
            char* keyword = token_now->text;
            token_stream_t condition = (token_stream_t)checked_malloc(sizeof( token_stream));
            token_t condition_head = create_token_with_type(HEAD, NULL, -1);
            token_now = token_now->next;
            condition_head -> next = token_now;
            condition->head = condition_head;
            condition->tail = condition_head;
            int count_do = 0;
            if (token_now->t_type == WORD &&( strcmp (token_now ->text, "do") ==0 ||strcmp (token_now ->text, "done") ==0  ) )
            {
              error(2, 0, "Line %d: Syntax error. incorrect token \"do\" or \"done\" ", line);
              return NULL;
            }
            while ((token_now->t_type == WORD && strcmp ( token_now->text, "do") != 0)||token_now->t_type != WORD)
            {
              if (token_now->t_type == WORD && (strcmp ( token_now->text, "then") == 0 || strcmp ( token_now->text, "if") == 0 || strcmp ( token_now->text, "done") == 0 ||
               strcmp ( token_now->text, "\n") == 0 || strcmp ( token_now->text, ";") == 0))
              {
                error(2, 0, "Line %d: Syntax error. incorrect token \"do\" or \"then\" or \"if\" .", line);
                return NULL;
              }
              condition->tail = token_now;
              token_now = token_now->next;
              if(token_now == NULL)
              {
                 error(2, 0, "Line %d: Syntax error. No do found", line);
                return NULL;
              }
            }
             if(token_now == NULL)
            ...
            count_do ++;
            condition->tail->next = NULL;
            condition->next = NULL;
            command_t condition_cmd= make_command_tree (condition);
            comm-> u.command[0] = condition_cmd;    //store the condition in u.command[0]
            //loopbody
            token_stream_t loopBody = (token_stream_t)checked_malloc(sizeof( token_stream));
            token_t loopBody_head = create_token_with_type(HEAD, NULL, -1);
            token_now = token_now->next;
            loopBody_head -> next = token_now;
            loopBody->head = loopBody_head;
            loopBody->tail = loopBody_head;
            if (token_now->t_type == WORD &&strcmp (token_now->text, "done") ==0 )
            {
              error(2, 0, "Line %d: Syntax error. incorrect token \"done\". ", line);
              return NULL;
            }
            while ((token_now->t_type == WORD && strcmp ( token_now->text, "done") != 0 )|| (count_do >=1)||token_now->t_type != WORD )
            {
              if (token_now ->t_type == WORD )
              {
                if (strcmp ( token_now->text, "if") == 0)
                {
                  if(token_now->prev->t_type != HEAD  )
                    {
                      
                     
                       token_t semi_token = create_token_with_type(SEMICOLON, NULL, 8);
                       token_now->prev->next = semi_token;
                       semi_token->prev = token_now->prev;
                       semi_token->next = token_now;
                       token_now->prev = semi_token;
                    }
                }
                if  (strcmp ( token_now->text, "while") == 0 ||strcmp ( token_now->text, "until") == 0||strcmp ( token_now->text, "for") == 0
                  )
               {
                  count_do ++;
                  if(token_now->prev->t_type != HEAD  )
            ...
            loopBody->tail->next = NULL;
            loopBody->next = NULL;
            command_t loopBody_cmd = make_command_tree (loopBody);
            comm->u.command[1] = loopBody_cmd;
            if (strcmp (keyword, "until") == 0)
              comm->type = UNTIL_COMMAND;
            else if (strcmp (keyword, "while") == 0)
              comm->type = WHILE_COMMAND;
            else if (strcmp (keyword, "for") == 0)
              comm->type = FOR_COMMAND;
            else 
              error(2, 0, "Line %d: error storing command type.", line);
            push (operands, comm);
            token_now = token_now->next;
          }
          ...
          //if command
          else if(token_now->t_type == WORD &&strcmp(token_now->text, "if")==0)
        {
          comm->type = IF_COMMAND;
          token_now = token_now->next;
          token_stream_t new =(token_stream_t)checked_malloc(sizeof(token_stream));
          token_t new_head_token = create_token_with_type(HEAD,NULL,-1);
          new->head = new_head_token;
          new_head_token->next = token_now;
          token_now->prev = new_head_token;
          int count_if = 0;
          if(token_now->t_type == WORD &&strcmp(token_now->text,"then") ==0)//if is directly followed by if
          {
            error(2, 0, "Line %d: Syntax error. Command cannot be empty", line);
            return NULL;
          }
          while((token_now->t_type == WORD &&strcmp(token_now->text,"then")!=0)||token_now->t_type != WORD)
          {

            if (token_now->t_type == WORD &&(strcmp ( token_now->text, "do") == 0 || 
              strcmp ( token_now->text, "if") == 0 || strcmp ( token_now->text, "done") == 0 || 
              strcmp ( token_now->text, "\n") == 0 || strcmp ( token_now->text, ";") == 0))
              {
                error(2, 0, "Line %d: Syntax error. incorrect token \"do\".", line);
                return NULL;
              }
            new->tail = token_now;
            token_now = token_now->next;
          ...
          if(token_now == NULL)
          {
             error(2, 0, "Line %d: Syntax error. Do not find then ", line);
              return NULL;
          }
          count_if ++;
          new->tail->next = NULL;
          new->next =NULL;
          command_t condition = make_command_tree(new);
          //token_now = token_now->next;
          comm->u.command[0] = condition;
          //then....else
          token_stream_t ifBody =  (token_stream_t)checked_malloc(sizeof( token_stream ));
          token_t ifBody_head = create_token_with_type(HEAD, NULL, -1);
          token_now = token_now->next;
          ifBody_head->next = token_now;
          ifBody->head = ifBody_head;
          ifBody->tail = ifBody_head;
          if (token_now->t_type == WORD &&(strcmp (token_now->text, "else") ==0 || strcmp (token_now->text, "fi") ==0||strcmp (token_now->text, "then") ==0))
            {
              error(2, 0, "Line %d: Syntax error. incorrect token \"else\" or  \"fi\". or \"then\" ", line);
              return NULL;
            }

            while ((token_now->t_type == WORD && strcmp ( token_now->text, "else") != 0 && strcmp ( token_now->text, "fi") != 0)|| (count_if >1)||token_now->t_type != WORD )
            {
              if (token_now ->t_type == WORD )
              {
                 if  (strcmp ( token_now->text, "while") == 0 ||strcmp ( token_now->text, "until") == 0||strcmp ( token_now->text, "for") == 0
              ...
              ifBody->tail = token_now;
              token_now = token_now->next;
            }
            ifBody->tail->next = NULL;
            ifBody->next = NULL;
            command_t ifBody_cmd = make_command_tree (ifBody);
            comm->u.command[1] = ifBody_cmd;
            //else...fi
            if (strcmp ( token_now->text, "else") == 0)
            {
              token_stream_t elseBody =  (token_stream_t)checked_malloc(sizeof( token_stream ));
              token_t elseBody_head = create_token_with_type(HEAD, NULL, -1);
              token_now = token_now->next;
              elseBody_head->next = token_now;
              elseBody->head = elseBody_head;
              elseBody->tail = elseBody_head;
             if (strcmp (token_now->text, "elif") ==0 || strcmp (token_now->text, "fi") ==0||strcmp (token_now->text, "then") ==0)
              {
                error(2, 0, "Line %d: Syntax error. incorrect token \"elif\" or  \"fi\". or \"then\" ", line);
                return NULL;
              }
             while ((token_now->t_type == WORD &&  strcmp ( token_now->text, "fi") != 0)|| (count_if >=1)||token_now->t_type != WORD )
            {
              if (token_now ->t_type == WORD )
              {
                 if  (strcmp ( token_now->text, "while") == 0 ||strcmp ( token_now->text, "until") == 0||strcmp ( token_now->text, "for") == 0
                ...
                if  (strcmp ( token_now->text, "if") == 0 )
               {
                ...
                elseBody->tail = token_now;
                token_now = token_now->next;
             }
              elseBody->tail->next = NULL;
              elseBody->next = NULL;
              command_t elseBody_cmd = make_command_tree (elseBody);
              comm->u.command[2] = elseBody_cmd;
            }
          push(operands,comm);
          token_now = token_now->next;
        }
...
{
  command_stream_t cmd_stream = (command_stream_t)checked_malloc(sizeof(command_stream));
		     ...
		     void *get_next_byte_argument)
{
  ...
  token_stream_t deleted_token_stream = delete_newline(return_token_stream);
 command_t new_command_t = make_command_tree (deleted_token_stream);
  build_depend_list(new_command_t,deleted_token_stream);
 ...
 // print_command(new_command_t);
    while (deleted_token_stream->next != NULL)
  {
    //make return_point point to next token stream
    deleted_token_stream = deleted_token_stream->next;
    //free(return_point);
    //return_token_stream = temp;
    new_command_t = make_command_tree (deleted_token_stream);
    build_depend_list(new_command_t,deleted_token_stream);
    add_command_to_stream(new_command_t);
