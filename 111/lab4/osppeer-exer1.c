...
#define FILE_REPEAT 2
#define UPLOAD_OVERRUN 3
 ...
 */
//Exer.2: chage task buffer size 
#define TASKBUFSIZ	 65536	// Size of task_t::buf
#define FILENAMESIZ	256	// Size of task_t::filename
 //added global:
 #define MAX_FILE_SIZE (1<<20)	//maximum file size：1gb
 #define MAX_FORK_PROCESS 10
...
typedef struct task {
	...
	char checksum[MD5_TEXT_DIGEST_SIZE+1];	//tracker checksum
} task_t;
...
static task_t *task_new(tasktype_t type)
{
	...
	//extra-credit: init checksum variables:
...
taskbufresult_t read_to_taskbuf(int fd, task_t *t)
{
	...
	//extra-credi: append download data to md5:
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	//extra-credit: obtain checksum:
	osp2p_writef (tracker_task->peer_fd, "MD5SUM %s\n", t->filename);
	messagepos = read_tracker_response(tracker_task);
	if (tracker_task->buf[messagepos] != '2') 
	{
		error("Tracker error requesting md5 checksum.");
		goto exit;
	}
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
		...
		//Exer.2: track the size of the file downloaded and limit the size of the file
		if (t->total_written> MAX_FILE_SIZE)
		{
			error("the downloading file is too big.");
			goto try_again;
		}
	}
	...
	if (t->total_written > 0) {
		...
		//compare file checksum and tracker checksum:
		char mdchecksum[MD5_TEXT_DIGEST_SIZE+1];
		mdchecksum[MD5_TEXT_DIGEST_SIZE] = '\0';
		if (md5_finish_text(t->md5_state,mdchecksum, 1) == MD5_TEXT_DIGEST_SIZE)
		{
			if (strncpy (t->checksum, mdchecksum, MD5_TEXT_DIGEST_SIZE) == 0)
				message("MD5 checksum verification success for file %s.\n", t->filename);
			else	//verification fails
			{	
				error("MD5 checksum verification does not pass for file %s.\n", t->filename);
				goto try_again;
			}
		}
		else 
		{
			error ("MD5 checksum length error for file %s.\n", t->filename);
			goto try_again;
		}
...
static void task_upload(task_t *t)
{
	...
	//Exer.2: check if the passed request is too long:
	...
	//Exer.2: check if the requested file name is too long:
	if (strlen (t->filename)>FILENAMESIZ)
	{
		error("requested filename too long.");
		goto exit;
	}
	t->head = t->tail = 0;
	//Exer.2: check if the file asked for is outside the current directory
	char file_path [PATH_MAX], current_path [PATH_MAX];
	char* cur_path;
	char* fi_path;
	cur_path = getcwd (current_path, PATH_MAX);
	fi_path = realpath(t->filename, file_path);
	if (cur_path == NULL)
	{
	...
	if (strncmp(cur_path, fi_path, strlen(cur_path))!=0)
	{
		error("the file is not in the current directory.");
		goto exit;
	}
		...
		if ((t = start_download(tracker_task, argv[1])))
			{
				...
				else 	// reach the maximum process limit
				{
					error ("can't fork: reach maximum process limit.");
					continue;
				}	
			}
