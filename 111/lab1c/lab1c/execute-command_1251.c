...
add_to_waitlist(command_t waited_command, command_t waiting_command)
{
	if(waited_command->waitlist_head == NULL)
  {
    ...
    waited_command->waitlist->prev = NULL;
    waited_command->waitlist->next = NULL;
  }
  else
  {
    waitlist_t waitlist_new = (waitlist_t)checked_malloc(sizeof(waitlist_t));
    waitlist_tail->next = waitlist_new;
    waitlist_new->prev = waitlist_tail;
    waitlist_tail = waitlist_new;
    waitlist_tail->next = NULL;
    waitlist_tail->cmd = waiting_command;
  }
}
int 
check_dependency (command_t cmd, outfile_t head)
{
	if (cmd == NULL || head == NULL||cmd->head_depend == NULL)
	{
