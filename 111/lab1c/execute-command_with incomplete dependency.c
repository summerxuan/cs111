...
check_dependency (command_t cmd, outfile_t head)
{
	...
	while (cmd -> dependency_text[i] != NULL)
	{
		outfile_t depend = head;
		while (depend != outfile_tail)
		{
			if (dependency_text[i] == depend->text)
			{
				//使cmd waitpid （depend->cmd 的process）
				dependency ++;	//has dependency, count+1
				break;	//跳出子循环；
			}
			depend = depend ->next;
		}
...
void set_command_status (command_t cmd, outfile_t head)
{
	...
	else 	// need to wait someone to exit
	{
		;
	}
}
