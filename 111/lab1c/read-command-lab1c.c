...
depend_list_t head_depend = NULL;
depend_list_t tail_depend = NULL;

depend_list_t
initDependlist(char* filename)
{
...
void
add_filename_to_dependlist(char* filename)
{
  
  if(tail_depend== NULL)
  {
    tail_depend = initDependlist(filename);
    head_depend= tail_depend;
  }
  else
  {
    depend_list_t depend_new = initDependlist(filename);
    //command_stream_head->next = command_stream_new;
    tail_depend->next = depend_new;
    tail_depend = tail_depend->next;
    tail_depend->next = NULL;
  }
}
    ...
    
    {
      ...
      while (  !is_empty(operators) && top(operators)->type == PIPE_COMMAND )
        ...
        case WORD:
          ...
          //add_filename_to_dependlist(comm->u.word[0]);
          int i =1;
          while(i< num_words)
          {
            token_now = token_now->next;
